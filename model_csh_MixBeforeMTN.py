"""
MixBeforeMTN:concat 后再过MTN&RTFM
"""
import torch
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if args.fusion == 'concat':
            self.Aggregate = Aggregate(len_feature=args.feature_size+args.emb_dim)  # early mix
            self.fc1 = nn.Linear(args.feature_size + args.emb_dim, 512)
        elif args.fusion == 'add' or args.fusion == 'product':
            self.Aggregate = Aggregate(len_feature=args.emb_dim)
    ...
    def forward(self, inputs, text):
        ...
        if self.fusion == 'concat':
            out=torch.cat((out,out2),dim=2)     # early mix out.shape=[640,32,2048+768]
