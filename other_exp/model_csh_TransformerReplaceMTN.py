...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.Aggregate = Transformer(dim=args.feature_size, depth=1,heads=4, mlp_ratio=1, dropout=0.5)
        self.Aggregate_text = Transformer(dim=args.emb_dim, depth=1,heads=4, mlp_ratio=1, dropout=0.5)
        # assert args.feature_size==args.emb_dim, f'args.feature_size {args.feature_size}!=args.emb_dim {args.emb_dim}'
        # self.Aggregate = Transformer(dim=args.feature_size, depth=1, heads=4, mlp_ratio=2, dropout=0.5)
        # self.Aggregate_text = self.Aggregate
        ...
        # self.selfatt = Transformer(dim=512, depth=2,heads=4, dim_head=128,mlp_dim=512, dropout=0.5)
    ...
    def forward(self, inputs, text):
        ...
        #######  process abnormal videos -> select top3 feature magnitude  #######c
