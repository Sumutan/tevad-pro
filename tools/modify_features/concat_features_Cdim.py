...
"""
def add_features(folder_A, folder_B, output_folder): #A is clip folder
    # 创建输出文件夹
    ...
    for file_name in os.listdir(folder_A):
        if file_name.endswith('.npy'):
            ...
            file_path_B = os.path.join(folder_B, file_name) #default
            ...
            # 残差连接
            if features_B.shape==features_A.shape:  # [crop,T,C]
                concatenated_features =(features_A+features_B)/2
                np.save(output_file_path, concatenated_features)
                print(f"mixed features saved to: {output_file_path}")
            else: #输出错误日志
...
if __name__ == '__main__':
    ...
    dataset='Crime' #Crime/TAD/Shanghai/Violence

    output_folder = f'/home/cw/sh/dataset/Crime/feature/8-26_8-22_SVMAE_wo-loss_CLIP-B/'
    ...
    #单次运行
    folder_B_fullpath = "/home/cw/sh/dataset/Crime/feature/8-26_8-22_SVMAE_wo-loss/"
    concatenate_features(CLIP_folder, folder_B_fullpath, output_folder)
